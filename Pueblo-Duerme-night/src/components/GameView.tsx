...
export default function GameView({ gameId, user, onLeave }: GameViewProps) {
  ...
  return (
    <div className={`transition-colors duration-500 h-screen flex flex-col p-6 font-sans overflow-hidden relative ${theme === 'sketchy' ? 'watercolor-bg text-[#2c1810]' : theme === 'night' ? 'text-[#e8d5b0]' : 'bg-slate-950 text-slate-200'}`} style={theme === 'night' ? {background:'transparent'} : {}}>
      <SketchyFilters />
      ...
      {/* Header Section */}
      <HandDrawn
        className={`flex justify-between items-center mb-6 h-20 px-8 shrink-0 ${
          theme === 'sketchy' ? 'bg-transparent shadow-none'
          : theme === 'night'  ? 'rounded-2xl'
          : 'bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl'
        }`}
        fill={theme === 'night' ? 'transparent' : 'transparent'}
        stroke={theme === 'sketchy' ? '#2c1810' : theme === 'night' ? '#3d2a14' : '#1e1b4b'}
        style={theme === 'night' ? {background:'#130f09', border:'1px solid #3d2a14', boxShadow:'0 0 30px rgba(0,0,0,0.5)'} as React.CSSProperties : undefined}
      >
        <div className="flex items-center gap-4">
          <div className={`h-3 w-3 rounded-full animate-pulse ${game.status === 'playing' ? (theme === 'night' ? 'night-pulse-red bg-red-900' : 'bg-red-600') : 'bg-emerald-500'}`}></div>
          <h1 className={`text-2xl leading-none italic ${
            theme === 'sketchy' ? 'text-[#2c1810] font-display text-3xl'
            : theme === 'night'  ? 'night-title text-2xl'
            : 'text-white font-display font-black uppercase tracking-widest'
          }`}>
            Pueblo <span className={theme === 'sketchy' ? 'text-amber-800' : theme === 'night' ? '' : 'text-indigo-400'}>Duerme</span>
          </h1>
        </div>
        
        <div className="flex items-center gap-6">
          <HandDrawn fill={theme === 'sketchy' ? '#fffbeb' : theme === 'night' ? 'rgba(0,0,0,0.3)' : '#1e293b'} className="px-4 py-2 rounded-xl hidden sm:block shrink-0">
            <span className={`text-[10px] uppercase block font-black leading-none mb-1 whitespace-nowrap ${theme === 'sketchy' ? 'font-chat text-lg py-1 text-slate-500' : theme === 'night' ? 'night-label' : 'text-slate-500'}`}>Código de Partida</span>
            <span className={`text-lg font-bold tracking-widest leading-none uppercase block ${theme === 'sketchy' ? 'text-amber-900 font-typewriter' : theme === 'night' ? 'night-code' : 'font-mono text-amber-400'}`}>{game.lobbyCode}</span>
          </HandDrawn>
          <div className={`px-6 py-2 rounded-xl shadow-lg text-xs sm:text-sm ${
            theme === 'sketchy' ? 'bg-amber-100/50 text-amber-900 border border-amber-900/20 font-typewriter'
            : theme === 'night'  ? 'night-phase-badge rounded-lg'
            : 'bg-indigo-600 text-white shadow-indigo-900/20 font-black uppercase tracking-tighter'
          }`}>
          ...
          <button
            ...
            className={`p-2 rounded-xl transition-all border ${
              theme === 'sketchy' ? 'bg-white border-amber-900/20 text-amber-900 hover:bg-amber-50'
              : theme === 'night'  ? 'night-btn-secondary rounded-lg'
      ...
      <main className="flex-1 grid grid-cols-12 gap-4 overflow-hidden min-h-0">
         {/* Table Area (Left) */}
         <section className={`col-span-12 lg:col-span-8 p-6 rounded-[2rem] flex flex-col items-center overflow-y-auto min-h-0 relative ${theme === 'sketchy' ? 'bg-white/40 border border-amber-900/10' : theme === 'night' ? 'border rounded-2xl' : 'bg-slate-900/50 border border-slate-800/50'}`} style={theme === 'night' ? {background:'rgba(19,15,9,0.7)', border:'1px solid #3d2a14', backdropFilter:'blur(4px)'} : undefined}>
            {theme === 'sketchy' && <PencilSketch type="wolf" className="absolute top-4 right-4 w-12 h-12 text-amber-900/10" />}
            {/* Phase / Narration */}
            <div className={`mb-8 w-full text-center shrink-0 ${theme === 'sketchy' ? 'py-6 rounded-sm border-b-2 border-amber-900/15 rotate-[0.3deg] shadow-sm' : ''}`}
              style={theme === 'sketchy' ? {background:'linear-gradient(180deg, rgba(255,245,220,0.6) 0%, rgba(240,225,195,0.3) 100%)'} : undefined}>
               ...
               {players.sort((a,b) => b.joinedAt - a.joinedAt).map((p, idx) => {
                 ...
                 return (
                   <HandDrawn 
                     ...
                     className={`relative border-2 rounded-2xl p-4 flex flex-col items-center justify-center transition-all group ${theme === 'sketchy' ? (idx % 2 === 0 ? 'rotate-1' : '-rotate-1') : 'shadow-xl'} ${canBeTarget ? (theme === 'night' ? 'hover:border-[#9b2020] cursor-pointer' : 'hover:border-indigo-500 cursor-pointer') : (theme === 'sketchy' ? 'border-amber-900/10' : theme === 'night' ? 'border-[#3d2a14]' : 'border-slate-800')} ${isActing ? (theme === 'sketchy' ? 'border-amber-500' : theme === 'night' ? 'border-[#d4860a]' : 'border-indigo-500') : ''} ${!p.isAlive ? 'opacity-50 grayscale' : ''} ${theme === 'sketchy' ? 'bg-white/40 shadow-none' : theme === 'night' ? '' : 'bg-slate-800/80'}`}
                     ...
                     style={theme === 'night' ? {background: isActing ? 'rgba(40,20,5,0.8)' : 'rgba(28,21,13,0.85)', border: `1px solid ${isActing ? '#d4860a' : canBeTarget ? '#5a3d1a' : '#2a1a0a'}`} as React.CSSProperties : undefined}
                   >
                     {/* Labels */}
                     <div className="absolute top-2 left-2 right-2 flex justify-between items-center px-1">
                        {isMe && <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full text-white uppercase ${theme === 'sketchy' ? 'bg-amber-600' : theme === 'night' ? 'night-you-badge' : 'bg-indigo-500'}`}>TÚ</span>}
                        ...
                        {p.isModerator && <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full border uppercase ${theme === 'sketchy' ? 'bg-amber-50 border-amber-500/30 text-amber-900' : theme === 'night' ? 'night-mod-badge' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>MOD</span>}
                     ...
                     <span className={`text-sm truncate w-full text-center ${theme === 'sketchy' ? 'font-chat text-2xl text-amber-950 px-2' : theme === 'night' ? 'night-body font-semibold' : 'font-bold text-slate-300'} ${!p.isAlive ? 'text-red-700 decoration-red-700/50 line-through' : ''}`}>
                        {p.displayName}
                      </span>
         ...
         {/* Sidebar (Right) */}
         <aside className="col-span-12 lg:col-span-4 flex flex-col gap-4 min-h-0 min-w-0 overflow-hidden">
            {/* Narrative / Chat Box */}
            <HandDrawn 
              className={`flex-1 border rounded-[3rem] p-6 flex flex-col shadow-inner overflow-hidden min-h-0 min-w-0 ${theme === 'sketchy' ? 'bg-white/40 border-amber-900/10 shadow-none bg-slate-900' : theme === 'night' ? 'rounded-2xl' : 'bg-slate-900 border-slate-800'}`}
              style={theme === 'night' ? {background:'rgba(13,10,7,0.75)', border:'1px solid #3d2a14', backdropFilter:'blur(4px)'} : undefined}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto overflow-x-hidden mb-4 space-y-3 scrollbar-hide pr-2 min-w-0 w-full">
                 {messages.filter(m => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded-2xl border transition-all ${
                          isNarrator 
                            ? (theme === 'sketchy' ? 'bg-amber-100 border-amber-300 shadow-sm' : theme === 'night' ? 'night-chat-narrator rounded-xl border-l-2 border-t-0 border-r-0 border-b-0' : 'bg-indigo-600/10 border-indigo-500/50 shadow-[0_0_15px_rgba(79,70,229,0.1)] scale-[1.02]')
                            : m.type === 'werewolf' 
                              ? (theme === 'sketchy' ? 'bg-red-50 border-red-200' : theme === 'night' ? 'night-chat-wolf rounded-xl border-l-2 border-t-0 border-r-0 border-b-0' : 'bg-red-950/20 border-red-900/30')
                              : (theme === 'sketchy' ? 'bg-white/60 border-amber-200' : theme === 'night' ? 'night-chat-public rounded-xl border-l-2 border-t-0 border-r-0 border-b-0' : 'bg-slate-950/40 border-slate-800')
                        }`}
                      >
                         <span className={`text-[10px] font-black uppercase mb-1 block ${
                           isNarrator 
                             ? (theme === 'sketchy' ? 'text-amber-800' : theme === 'night' ? 'text-[#d4860a]' : 'text-indigo-300') 
                             : m.type === 'werewolf' 
                               ? (theme === 'night' ? 'text-red-700' : 'text-red-600')
                               : (theme === 'sketchy' ? 'text-amber-700' : theme === 'night' ? 'text-[#7a6045]' : 'text-indigo-400')
                         }`}>
                            {m.senderName} {m.type === 'werewolf' && '🌑'}
                         </span>
                         <p className={`text-sm break-words leading-relaxed ${isNarrator ? (theme === 'sketchy' ? 'text-amber-950 font-medium italic font-chat' : theme === 'night' ? 'night-narration-text text-sm' : 'text-white font-medium italic') : (theme === 'sketchy' ? 'text-amber-900 font-chat' : theme === 'night' ? 'night-body text-sm' : 'text-slate-300')}`}>
                            {m.text}
                         </p>
                      </div>
                    );
              ...
              {(activeSecret?.role === 'werewolf' || game.phase === 'day_vote' || game.phase === 'day_reveal' || game.phase === 'lobby' || isMod) && (
                <form onSubmit={sendMsg} className="relative group shrink-0">
                   <input 
                     ...
                     className={`w-full border rounded-2xl px-5 py-4 text-sm transition-all shadow-xl ${
                       theme === 'sketchy' 
                         ? 'bg-white border-amber-900/20 text-amber-950 placeholder:text-amber-900/40 focus:ring-amber-500/30 font-chat' 
                         : theme === 'night'
                           ? 'night-input rounded-xl'
                   ...
                   <button type="submit" className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-xl transition-all shadow-lg active:scale-90 ${
                     theme === 'sketchy' ? 'bg-amber-600 text-white hover:bg-amber-500' : theme === 'night' ? 'night-btn-primary rounded-lg' : 'bg-indigo-600 text-white hover:bg-indigo-500'
                   }`}>
       ...
       <footer className="mt-6 flex flex-col lg:flex-row gap-4 h-auto lg:h-24 shrink-0">
         <HandDrawn 
           className={`flex-1 border rounded-3xl flex flex-wrap items-center px-6 py-4 gap-6 min-w-0 shadow-2xl relative overflow-hidden ${theme === 'sketchy' ? 'bg-white/60 border-amber-900/20 shadow-none bg-slate-900' : theme === 'night' ? '' : 'bg-slate-900 border-slate-800'}`}
           style={theme === 'night' ? {background:'rgba(19,15,9,0.85)', border:'1px solid #3d2a14', backdropFilter:'blur(4px)'} : undefined}
         ...
         <HandDrawn className={`w-full lg:w-64 rounded-3xl flex items-center justify-center gap-4 py-4 px-6 shadow-2xl shrink-0 ${theme === 'sketchy' ? 'bg-white/60 border-amber-900/20 shadow-none bg-slate-900 border border-slate-800' : theme === 'night' ? '' : 'bg-slate-900 border border-slate-800'}`}
           style={theme === 'night' ? {background:'rgba(19,15,9,0.85)', border:'1px solid #3d2a14'} as React.CSSProperties : undefined}>
           <div className="flex -space-x-2">
             {players.slice(0, 3).map((p, i) => (
               <div key={i} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-black uppercase shadow-lg ${theme === 'sketchy' ? 'bg-amber-100 border-amber-950/20 text-amber-700' : theme === 'night' ? 'text-[#7a6045]' : 'bg-slate-800 border-slate-950 text-slate-500'}`}
                 style={theme === 'night' ? {background:'#1c150d', border:'1px solid #3d2a14'} as React.CSSProperties : undefined}>
                 {p.displayName[0]}
               </div>
             ))}
             {players.length > 3 && (
               <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-black ${theme === 'sketchy' ? 'bg-amber-200 border-amber-950/20 text-amber-800' : theme === 'night' ? 'text-[#5a3d1a]' : 'bg-slate-950 border-slate-900 text-slate-400'}`}
                 style={theme === 'night' ? {background:'#130f09', border:'1px solid #3d2a14'} as React.CSSProperties : undefined}>
                 +{players.length - 3}
               </div>
             )}
           </div>
           <span className={`text-[10px] font-black uppercase tracking-widest leading-none ${theme === 'sketchy' ? 'text-amber-900/40' : theme === 'night' ? 'night-label' : 'text-slate-500'}`}>{players.length} EN LÍNEA</span>
         </HandDrawn>
       </footer>
      ...
      <AnimatePresence>
        {((game.phase === 'night_start' || (game.phase.includes('turn') && !isMyTurn)) && 
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`fixed inset-0 z-50 flex flex-col items-center justify-center p-6 text-center ${theme === 'sketchy' ? 'watercolor-bg text-amber-950 font-typewriter' : theme === 'night' ? 'text-[#e8d5b0]' : 'bg-black text-white'}`}
            style={theme === 'night' ? {background:'rgba(5,3,2,0.97)'} as React.CSSProperties : undefined}
          >
             <SketchyFilters />
             {theme === 'night' && (
               <svg width="120" height="120" viewBox="0 0 120 120" className="mb-8 night-torch-glow">
                 <defs><filter id="mng"><feTurbulence type="fractalNoise" baseFrequency="0.06" numOctaves="3" seed="2" result="n"/><feDisplacementMap in="SourceGraphic" in2="n" scale="4" xChannelSelector="R" yChannelSelector="G"/></filter></defs>
                 <circle cx="60" cy="60" r="45" fill="none" stroke="#3d2a14" strokeWidth="1" filter="url(#mng)"/>
                 <circle cx="60" cy="60" r="35" fill="none" stroke="#5a3d1a" strokeWidth="0.8" filter="url(#mng)" opacity="0.6"/>
                 <path d="M75 38 Q95 60 75 82 Q62 75 62 60 Q62 45 75 38Z" fill="#d4c090" opacity="0.45" filter="url(#mng)"/>
                 {[0,45,90,135,180,225,270,315].map((a,i)=>(
                   <circle key={i} cx={60+52*Math.cos(a*Math.PI/180)} cy={60+52*Math.sin(a*Math.PI/180)} r="1.2" fill="#d4860a" opacity="0.4"/>
                 ))}
               </svg>
             )}
             {theme !== 'night' && <Moon className={`w-24 h-24 mb-8 animate-pulse ${theme === 'sketchy' ? 'text-amber-900' : 'text-indigo-500'}`} />}
             <h1 className={`text-4xl md:text-6xl font-black italic mb-4 uppercase ${theme === 'sketchy' ? 'font-typewriter' : theme === 'night' ? 'night-title' : 'font-display drop-shadow-[0_0_15px_rgba(99,102,241,0.5)]'}`}
               style={theme === 'night' ? {fontSize:'3rem'} as React.CSSProperties : undefined}>
               EL PUEBLO <span>DUERME</span>
             </h1>
             <p className={`max-w-md ${theme === 'sketchy' ? 'font-chat text-3xl text-amber-900/70' : theme === 'night' ? 'night-narration-text' : 'text-slate-400 font-bold'}`}>{game.narration}</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {game.status === 'ended' && (
          <motion.div 
            ...
            className={`fixed inset-0 z-[60] flex flex-col items-center justify-center p-6 text-center ${theme === 'sketchy' ? 'bg-amber-50/90' : theme === 'night' ? 'bg-black/85 backdrop-blur-sm' : 'bg-slate-950/80'}`}
          >
             <HandDrawn
               className={`p-12 rounded-[3rem] max-w-lg ${theme === 'sketchy' ? 'bg-transparent shadow-none' : theme === 'night' ? (game.winner === 'lobos' ? 'night-winner-wolf rounded-2xl' : 'night-winner-village rounded-2xl') : 'bg-slate-900 border-slate-800 shadow-[0_0_100px_rgba(79,70,229,0.2)]'}`}
               fill="transparent"
               stroke={theme === 'sketchy' ? '#2c1810' : theme === 'night' ? (game.winner === 'lobos' ? '#9b2020' : '#2a5a2a') : '#1e1b4b'}
             >
                ...
                <h1 className={`text-5xl italic mb-4 tracking-tighter uppercase leading-none ${theme === 'sketchy' ? 'text-amber-950 font-typewriter' : theme === 'night' ? 'night-title' : 'text-white font-display font-black'}`}
                  style={theme === 'night' ? {fontSize:'2.8rem'} as React.CSSProperties : undefined}>
                  ¡PARTIDA <span className={game.winner === 'lobos' ? 'text-red-500' : 'text-emerald-500'}>FINALIZADA</span>!
                </h1>
                <p className={`text-xl font-bold mb-8 ${theme === 'sketchy' ? 'text-amber-900 font-chat text-3xl' : theme === 'night' ? 'night-narration-text' : 'text-slate-300'}`}>{game.narration}</p>
                
                {isMod && (
                  <button 
                    onClick={handleRestartGame}
                    className={`w-full font-black py-4 rounded-2xl shadow-lg transition-all active:scale-95 ${theme === 'sketchy' ? 'bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-900/30 font-typewriter' : theme === 'night' ? 'night-btn-primary rounded-xl py-4' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-900/40'}`}
                  >
                    JUGAR DE NUEVO
                  </button>
                )}
