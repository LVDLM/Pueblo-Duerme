...
export default function GameView({ gameId, user, onLeave }: GameViewProps) {
  ...
  return (
    <div className="h-screen flex flex-col p-6 font-sans overflow-hidden relative bg-[#fdfbf7] text-[#2c3e50]">
      {/* Background and particle effects are handled in App.tsx wrapper */}
      
      {/* Header Section */}
      <div className="flex justify-between items-center mb-6 h-20 px-8 shrink-0 bg-white/80 backdrop-blur-md border-4 border-[#4a3728]/10 rounded-[2rem] shadow-xl relative z-20">
        <div className="flex items-center gap-4">
          <div className={`h-3 w-3 rounded-full animate-pulse ${game.status === 'playing' ? 'bg-red-600' : 'bg-emerald-500'}`}></div>
          <h1 className="text-2xl font-black uppercase tracking-tight text-[#4a3728] font-display">
            Pueblo <span className="text-orange-600 italic">Duerme</span>
          </h1>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="px-4 py-2 bg-[#fdfbf7] border-2 border-[#4a3728]/10 rounded-xl hidden sm:block">
            <span className="text-[10px] uppercase text-[#4a3728]/50 block font-black leading-none mb-1">Código de Partida</span>
            <span className="text-lg font-bold tracking-widest leading-none uppercase text-[#4a3728] font-mono">{game.lobbyCode}</span>
          </div>
          <div className="bg-[#4a3728] text-white px-6 py-2 rounded-xl shadow-lg text-xs sm:text-sm font-black uppercase tracking-widest">
            Fase: {PHASE_LABELS[game.phase] || game.phase}
          </div>
          <button 
            onClick={onLeave}
            className="p-2 rounded-xl transition-all border-2 border-[#4a3728]/10 bg-white text-[#4a3728] hover:bg-orange-50 hover:text-orange-600"
          >
      ...
      <main className="flex-1 grid grid-cols-12 gap-4 overflow-hidden min-h-0">
         {/* Table Area (Left) */}
         <section className="col-span-12 lg:col-span-8 p-6 rounded-[2rem] flex flex-col items-center overflow-y-auto min-h-0 relative bg-white/40 border-2 border-[#4a3728]/5 shadow-inner">
            {/* Phase / Narration */}
            <div className="mb-8 w-full text-center shrink-0">
               <AnimatePresence mode="wait">
                  ...
                  >
                    {isMyTurn && (
                       <div className="mb-2 inline-block px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest animate-bounce bg-orange-600 text-white shadow-lg">
                          ¡Tu turno ha comenzado!
                       </div>
                    )}
                    <h3 className="text-3xl md:text-4xl leading-tight font-black text-[#4a3728] px-4">
                       "{game.narration}"
                    </h3>
                  </motion.div>
               </AnimatePresence>
            ...
            {/* Players Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-6 w-full p-2">
               {players.sort((a,b) => b.joinedAt - a.joinedAt).map((p, idx) => {
                 ...
                 return (
                   <motion.div 
                     ...
                     whileHover={canBeTarget ? { scale: 1.02, y: -5 } : {}}
                     className={`relative border-4 rounded-3xl p-6 flex flex-col items-center justify-center transition-all bg-white shadow-md ${canBeTarget ? 'hover:border-orange-500 cursor-pointer' : 'border-transparent'} ${isActing ? 'border-orange-600 shadow-orange-600/10' : ''} ${!p.isAlive ? 'opacity-40 grayscale' : ''}`}
                   >
                     {/* Labels */}
                     <div className="absolute top-3 left-3 right-3 flex justify-between items-center">
                        {isMe && <span className="text-[8px] font-black px-2 py-1 rounded-full bg-orange-600 text-white uppercase tracking-tighter shadow-sm">TÚ</span>}
                        {p.uid === impersonateId && <span className="bg-amber-400 text-[8px] font-black px-2 py-1 rounded-full text-[#4a3728] uppercase tracking-tighter shadow-sm">SUPLANTADO</span>}
                        {p.isModerator && <span className="text-[8px] font-black px-2 py-1 rounded-full bg-[#4a3728] text-white uppercase tracking-tighter shadow-sm">MOD</span>}
                        <div className="flex items-center gap-1">
                                ...
                                ) && <Heart className="w-4 h-4 fill-rose-500 text-rose-500 shadow-sm" />}
                                {isMod && game.nightTargets?.werewolfTarget === p.uid && <Skull className="w-4 h-4 fill-red-600 text-red-600 animate-pulse" />}
                             </div>
                     </div>

                     {/* Voting Indicator */}
                     {game.phase === 'day_vote' && players.filter(v => v.vote === p.uid).length > 0 && (
                       <div className="absolute -top-3 -right-3 z-10 bg-[#4a3728] text-white text-xs font-black w-8 h-8 flex items-center justify-center rounded-full shadow-lg border-2 border-white">
                     ...
                     {((activeSecret?.role === 'witch' && game.phase === 'witch_turn') || isMod) && game.nightTargets?.werewolfTarget === p.uid && (
                        <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 z-20 bg-red-600 text-white text-[8px] font-black px-3 py-1 rounded-full border-2 border-white whitespace-nowrap shadow-md">
                           VÍCTIMA LOBOS
                        </div>
                     )}
                     ...
                     {/* Card Graphic */}
                     <div className={`
                        relative w-24 h-32 md:w-28 md:h-36 rounded-2xl border-4 transition-all duration-500 preserve-3d mb-3 shadow-sm
                        ${(showRole && isActing) || !p.isAlive ? 'rotate-y-180' : ''}
                        ${p.isAlive ? 'border-[#4a3728]/10 bg-[#fdfbf7]' : 'border-red-900/10 bg-red-50/50'}
                     `}>
                        {/* Back */}
                        <div className="absolute inset-0 backface-hidden flex items-center justify-center rounded-xl overflow-hidden">
                           ...
                           {!p.isAlive && (
                              <div className="absolute inset-0 bg-red-950/20 flex items-center justify-center">
                                 <Skull className="w-10 h-10 text-red-600/80" />
                              </div>
                           )}
                        </div>
                        ...
                        {(isActing || (devPeekSecrets && allSecrets[p.uid]) || !p.isAlive) && (
                           <div className={`absolute inset-0 backface-hidden rounded-xl overflow-hidden ${(isActing || !p.isAlive) ? 'rotate-y-180' : ''}`}>
                              ...
                              />
                              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col items-center justify-end p-2 text-center pb-4">
                                 <span className="text-[7px] font-black text-white/70 uppercase leading-none mb-1 tracking-widest">
                                    {!p.isAlive ? 'Revelación' : isActing ? 'TU ROL' : 'SOPLO'}
                                 </span>
                                 <span className="text-sm font-black text-white uppercase leading-tight tracking-tight">
                     ...
                     <span className={`text-base font-black truncate w-full text-center text-[#4a3728] ${!p.isAlive ? 'text-red-700/50 decoration-red-700/30 line-through' : ''}`}>
                        {p.displayName}
                      </span>
                      {!p.isAlive && <span className="text-[8px] font-black uppercase mt-1 text-red-600 tracking-widest">ELIMINADO</span>}
                     {isDev && p.uid !== user.uid && (
                        <button 
                          ...
                          className={`mt-4 text-[7px] font-black uppercase px-4 py-2 rounded-xl transition-all shadow-sm ${impersonateId === p.uid ? 'bg-orange-600 text-white' : 'bg-[#4a3728]/10 text-[#4a3728] hover:bg-orange-600 hover:text-white'}`}
                        >
         ...
         <aside className="col-span-12 lg:col-span-4 flex flex-col gap-4 min-h-0">
            {/* Narrative / Chat Box */}
            <div className="flex-1 bg-white border-2 border-[#4a3728]/10 rounded-[3rem] p-6 flex flex-col shadow-inner overflow-hidden min-h-0">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-2 text-[#4a3728]/60">
              ...
              <div className="flex-1 overflow-y-auto mb-4 space-y-3 scrollbar-hide pr-2">
                        ...
                        className={`p-4 rounded-2xl border-2 transition-all ${
                          isNarrator 
                            ...
                            : m.type === 'werewolf' 
                              ...
                              : 'bg-[#fdfbf7] border-[#4a3728]/5'
                        }`}
                      >
                         <span className={`text-[9px] font-black uppercase mb-1 block ${
                           isNarrator 
                             ? 'text-orange-700' 
                             : m.type === 'werewolf' 
                               ? 'text-red-700' 
                         ...
                         <p className={`text-sm font-bold break-words leading-relaxed ${isNarrator ? 'text-orange-950 italic' : 'text-[#2c3e50]'}`}>
                            {m.text}
                         </p>
                      </div>
                    );
                 ...
                 <div ref={chatEndRef} />
                 {messages.length === 0 && (
                    <div className="bg-[#fdfbf7] p-4 rounded-2xl border-2 border-dashed border-[#4a3728]/10 italic text-[#4a3728]/40 text-xs font-bold">
              ...
              {(activeSecret?.role === 'werewolf' || game.phase === 'day_vote' || game.phase === 'day_reveal' || game.phase === 'lobby' || isMod) && (
                <form onSubmit={sendMsg} className="relative group shrink-0">
                   <input 
                     ...
                     placeholder={isDevNarratorMode ? "Habla como Narrador..." : "Escribe un mensaje..."}
                     className="w-full bg-[#fdfbf7] border-2 border-[#4a3728]/10 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:border-orange-500/30 transition-all shadow-sm"
                   />
                   <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-[#4a3728] text-white rounded-xl hover:bg-orange-600 transition-all shadow-md active:scale-90">
       ...
       <footer className="mt-6 flex flex-col lg:flex-row gap-4 h-auto lg:h-24 shrink-0">
         <div className="flex-1 bg-white border-4 border-[#4a3728]/10 rounded-[2rem] flex flex-wrap items-center px-6 py-4 gap-6 min-w-0 shadow-lg relative overflow-hidden">
           {/* Actions / Powers */}
         ...
         <div className="w-full lg:w-64 bg-white border-4 border-[#4a3728]/10 rounded-[2rem] flex items-center justify-center gap-4 py-4 px-6 shadow-sm shrink-0">
           <div className="flex -space-x-2">
             {players.slice(0, 3).map((p, i) => (
               <div key={i} className="w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-black uppercase shadow-sm bg-[#4a3728] text-white">
                 {p.displayName[0]}
               </div>
             ))}
             {players.length > 3 && (
               <div className="w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-black bg-[#fdfbf7] text-[#4a3728]">
                 +{players.length - 3}
               </div>
             )}
           </div>
           <span className="text-[10px] font-black uppercase tracking-widest leading-none text-[#4a3728]/40">{players.length} EN LÍNEA</span>
         </div>
       </footer>
      ...
      <AnimatePresence>
        {((game.phase === 'night_start' || (game.phase.includes('turn') && !isMyTurn)) && 
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex flex-col items-center justify-center p-6 text-center bg-slate-950/95 text-white"
          >

             <Moon className="w-24 h-24 mb-8 text-indigo-500 animate-pulse" />
             <h1 className="text-4xl md:text-6xl font-black italic mb-4 uppercase font-display drop-shadow-[0_0_15px_rgba(99,102,241,0.5)]">
               EL PUEBLO DUERME
             </h1>
             <p className="max-w-md text-slate-400 font-bold">{game.narration}</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {game.status === 'ended' && (
          <motion.div 
            ...
            className="fixed inset-0 z-[150] flex flex-col items-center justify-center p-6 text-center bg-orange-950/40"
          >
             <div className="bg-white border-[10px] border-[#4a3728] p-12 rounded-[4rem] max-w-2xl shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-4 bg-orange-600"></div>
                <div className={`mb-8 inline-flex p-6 rounded-[2rem] shadow-lg ${game.winner === 'lobos' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                   {game.winner === 'lobos' ? <Skull className="w-20 h-20" /> : <Sun className="w-20 h-20" />}
                </div>
                <h1 className="text-5xl md:text-7xl italic mb-6 tracking-tighter uppercase leading-none text-[#4a3728] font-black font-display">
                  ¡PARTIDA <span className={game.winner === 'lobos' ? 'text-red-500' : 'text-emerald-500'}>FINALIZADA</span>!
                </h1>
                <p className="text-2xl font-black mb-12 text-[#2c3e50] leading-tight max-w-md mx-auto italic">{game.narration}</p>
                
                {isMod && (
                  <button 
                    onClick={handleRestartGame}
                    className="w-full font-black py-6 rounded-3xl shadow-xl transition-all active:scale-95 bg-orange-600 hover:bg-orange-500 text-white text-xl uppercase tracking-widest border-4 border-[#4a3728]/10"
                  >
                    JUGAR DE NUEVO
                  </button>
                )}
